...
def test_interval_mapping():
    ...
    intervals = [(1.0, 2.0), (3.0, 3.5)]
    vocal, non_vocal = intervals_to_encoder_frame_lists(intervals, wav_len, sr, conv)
    assert len(vocal) == 2, vocal
    assert all(len(v) >= 2 for v in vocal)
    assert len(non_vocal) > 0
    print("OK test_interval_mapping:", [len(v) for v in vocal], "non_vocal", len(non_vocal))
